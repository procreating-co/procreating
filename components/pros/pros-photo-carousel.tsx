 ...
 * (`features-section.tsx`/`PhotoCarousel`), mas SEM link pra galeria (aqui são `<div>`, não `<a>`
 * — nada clicável) e avançando sozinho a cada 2s em vez de arrastar manualmente.
 *
      ...
      <Reveal>
        <div ref={trackRef} aria-hidden="true" className="flex gap-4 overflow-hidden px-6 lg:px-12">
          {doubled.map((photo, i) => (
            ...
            >
              <Image src={photo.src} alt={photo.alt} fill sizes="(min-width: 1024px) 38vw, (min-width: 640px) 46vw, 78vw" loading="lazy" className="object-cover opacity-90" />
            </div>
          ))}
        </div>
      </Reveal>
