...
export function ProsFinalCta({ headline, paragraph, ctaLabel, whatsapp, slug }: { headline: string; paragraph: string; ctaLabel: string; whatsapp: WhatsappConfig; slug: string }) {
  const href = `https://wa.me/${whatsapp.phoneDigits}?text=${encodeURIComponent(whatsapp.message)}`;
  return (
    <section id="contato" aria-label="Fale com a Procreating" className="bg-black px-6 py-32 lg:px-12 lg:py-44">
      <div className="mx-auto flex max-w-3xl flex-col items-center gap-8 text-center">
        ...
        <Reveal delayMs={100}>
          <p className="max-w-xl text-balance text-base leading-relaxed text-white/55 sm:text-lg">{paragraph}</p>
        </Reveal>
        <Reveal delayMs={200}>
          <a
            href={href}
