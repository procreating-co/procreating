...
/**
 * Operação de conteúdo — o fluxo mensal que transforma a estratégia em entrega concreta
 * (planejamento → pauta → roteiro → produção → edição → publicação). Mesmo padrão visual da
 * seção de programa da Elenita (passos em sequência, conectados por setas), mas aqui é o fluxo
 * real de produção de conteúdo da Pascoal — não um programa de TV.
 */
export function ProposalPascoalOperacao({ content, accent }: { content: PascoalProposalContent["operacao"]; accent: string }) {
  return (
