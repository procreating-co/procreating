...
/**
 * Upsell — mesmo padrão de add-ons usado por produtos SaaS premium: o preço fica oculto até o
 * item ser selecionado (só então revela, com uma animação curta) e soma a um total dinâmico
 * sempre visível. `basePrice` é a base estratégica (proposal-budget.tsx) — o total aqui começa
 * nela e sobe conforme os add-ons são marcados.
 */
export function ProposalUpsell({ content, basePrice, accent }: { content: ProposalContent["upsell"]; basePrice: number; accent: string }) {
  const [selected, setSelected] = useState<Record<string, boolean>>({});

  const total = useMemo(() => {
    return content.items.reduce((sum, item) => (selected[item.id] ? sum + item.price : sum), basePrice);
  }, [content.items, selected, basePrice]);
          ...
          {content.items.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0 text-right font-mono text-sm" style={{ color: accent }}>
                  <AnimatePresence mode="wait">
                    {active && (
                      <motion.span key="price" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }} transition={{ duration: 0.25 }}>
