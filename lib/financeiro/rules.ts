 ...
 * divide igualmente entre os `users.role === "owner"` sem override. Sem nenhuma linha em
 * `partner_shares`, isso é só "divide igual entre todos os owners" — o "50/50 por padrão" que
...
export async function computeDistribution(revenue: number): Promise<DistributionResult> {
  ...
  const [{ data: owners }, { data: shares }] = await Promise.all([
    supabase.from("users").select("*").eq("role", "owner"),
