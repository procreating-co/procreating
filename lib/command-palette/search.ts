...
const EMPTY_RESULTS: CommandPaletteResults = { clients: [], leads: [], tasks: [] };

/** Busca do ⌘K — clientes, leads e tarefas por nome/título, 5 de cada, mais recente primeiro.
 *  Server Action (não uma query client-side direta) pra não expor mais superfície de acesso ao
 *  Supabase no bundle do cliente do que o resto do produto já expõe. */
export async function searchCommandPaletteAction(query: string): Promise<CommandPaletteResults> {
  ...
  const [{ data: clients }, { data: leads }, { data: tasks }] = await Promise.all([
  ...
  return { clients: clients ?? [], leads: leads ?? [], tasks: tasks ?? [] };
}
