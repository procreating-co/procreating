-- Achado pelo próprio teste ponta a ponta ("Tasks aparecerem no Workspace" — item explícito do
-- teste final pedido): as 3 tasks que o onboarding cria não tinham due_date. `/meu-dia` só
-- mostra tarefas com `due_date <= hoje` (`listTodayAndOverdueTasks`) — sem data, elas nascem e
-- ficam invisíveis pra sempre, embora existam no banco. Mesma cuidado de fuso de `lib/date.ts`
-- (§19 da auditoria), agora em SQL: `now() AT TIME ZONE 'America/Sao_Paulo'` converte o instante
-- pro calendário de Brasília antes de extrair o dia — nunca `now()::date` cru (isso pegaria o
-- dia em UTC do servidor).
