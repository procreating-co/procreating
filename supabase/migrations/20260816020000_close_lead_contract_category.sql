-- `contracts.category` (migration anterior) virou NOT NULL — esta função é o único lugar do
-- código vivo que faz `insert into public.contracts` fora de seed/correção manual (todo
-- fechamento de negócio pela UI passa por aqui). Sem este ajuste, fechar qualquer lead quebraria
-- a partir de agora (`null value in column "category" violates not-null constraint`). Contrato
-- recém-criado no onboarding nasce sempre `status='ativo'` — mesma regra de backfill da migration
-- anterior, só que decidida na hora da criação em vez de inferida depois: recorrente →
-- 'recorrente_ativo', pontual → 'pontual_em_andamento' (acabou de fechar, ainda não foi entregue).
...
begin
  ...
  v_contract_category := case when v_contract_type = 'recorrente' then 'recorrente_ativo' else 'pontual_em_andamento' end;
  ...
  insert into public.contracts (
    client_id, type, status, category, start_date, end_date, monthly_value, due_day, auto_renew,
    total_value, payment_terms, special_conditions, created_by
  )
  values (
    v_client_id, v_contract_type, 'ativo', v_contract_category, v_start, v_end, v_monthly_value, v_due_day,
  ...
  -- `due_date` NOVO — sem isto as 3 tasks nasciam e nunca apareciam em "Tarefas de hoje"
  -- (`listTodayAndOverdueTasks` exige `due_date <= hoje`; NULL nunca satisfaz isso).
  insert into public.tasks (title, assignee_id, due_date, context_type, context_id, created_by)
  values
    ('Enviar contrato assinado', auth.uid(), v_today, 'client_onboarding', v_client_id, auth.uid()),
    ('Agendar reunião de kickoff', auth.uid(), v_today + 3, 'client_onboarding', v_client_id, auth.uid()),
    ('Configurar acesso do cliente', auth.uid(), v_today, 'client_onboarding', v_client_id, auth.uid());
